...
1. **及时提交**：每完成一项实质性改动立即 `git commit`（conventional commits），不留悬挂改动
2. **版本记录**：每个子项目在 `docs/CHANGELOG.md` 记录每次提交（时间、版本号 = commit 短哈希、主要修改内容），提交后立即补记；CHANGELOG 自身的维护提交不记录
3. **发布伴随进度更新**：构建+部署（发布）后同步更新对应子项目的 `docs/PROGRESS.md`
4. **构建仅在用户明确要求时执行**（禁止自动构建）
5. **构建正式版时同步推送**：用户说「构建正式版」，构建完成后立即 `git push` 推送当前分支
