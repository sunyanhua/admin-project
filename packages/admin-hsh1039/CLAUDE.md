...
"1039俱乐部+"管理后台。基于 React + Ant Design 构建，**PC端WEB为主，兼容手机端**。
...
> 项目进度（已完成模块、接口清单、待办）见 `PROGRESS.md`。
...
接口定义和响应格式以 `hsh-swagger.json` 为准，禁止凭旧项目经验猜测字段名。禁止使用 `docs/openapi.json`（旧系统搭子计划，完全不兼容）。
