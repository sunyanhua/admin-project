...
export const configApi = {
  ...
  // 获取社区积分配置（v6）
  getRewardConfig: (params: { name?: string; status?: number; start?: number; length?: number; keyword?: string }) => {
  ...
  // 获取社区设置（v6）
  getCommunitySettings: (params: { name?: string; start?: number; length?: number; keyword?: string }) => {
