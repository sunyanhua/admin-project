...
export const lotteryApi = {
  ...
  getPools: (params?: { status?: number; keyword?: string; page?: number; size?: number }) => {
    return request.get('/admin/v1/lottery/pool', { params });
  },
  getPoolDetail: (id: string) => {
    return request.get(`/admin/v1/lottery/pool/${id}`);
  },
  createPool: (data: CreatePoolRequest) => {
    return request.post('/admin/v1/lottery/pool', data);
  },
  updatePool: (id: string, data: UpdatePoolRequest) => {
    return request.put(`/admin/v1/lottery/pool/${id}`, data);
  },
  togglePoolStatus: (id: string, status: number) => {
    return request.patch(`/admin/v1/lottery/pool/${id}/status`, { status });
  },
  deletePool: (id: string) => {
    return request.delete(`/admin/v1/lottery/pool/${id}`);
  },

  // === 奖品 ===
  getPrizes: (poolId: string, params?: { status?: number; keyword?: string; page?: number; size?: number }) => {
    return request.get(`/admin/v1/lottery/pool/${poolId}/prize`, { params });
  },
  getPrizeDetail: (id: string) => {
    return request.get(`/admin/v1/lottery/prize/${id}`);
  },
  createPrize: (poolId: string, data: CreatePrizeRequest) => {
    return request.post(`/admin/v1/lottery/pool/${poolId}/prize`, data);
  },
  updatePrize: (id: string, data: UpdatePrizeRequest) => {
    return request.put(`/admin/v1/lottery/prize/${id}`, data);
  },
  togglePrizeStatus: (id: string, status: number) => {
    return request.patch(`/admin/v1/lottery/prize/${id}/status`, { status });
  },
  deletePrize: (id: string) => {
    return request.delete(`/admin/v1/lottery/prize/${id}`);
  },
  deployPrize: (id: string, data: DeployRequest) => {
    return request.post(`/admin/v1/lottery/prize/${id}/deploy`, data);
  },

  // === 投放批次 ===
  getDeployBatches: (params?: { status?: number; keyword?: string; page?: number; size?: number }) => {
    return request.get('/admin/v1/lottery/deployment/batch', { params });
  },

  // === 中奖记录 ===
  getUserPrizes: (params?: { status?: number; keyword?: string; page?: number; size?: number }) => {
    return request.get('/admin/v1/lottery/user-prize', { params });
  },
  shipUserPrize: (id: string, data: { ship_status: number; carrier?: string; tracking_number?: string }) => {
    return request.put(`/admin/v1/lottery/user-prize/${id}/ship`, data);
  },
};
