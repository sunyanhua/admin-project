...
1. **及时提交**：每完成一项实质性改动立即 `git commit`（conventional commits），不留悬挂改动（OldSun 同步可能覆盖未提交修改）
2. **版本记录**：每次提交后立即在 `docs/CHANGELOG.md` 顶部补一行（时间、版本号 = commit 短哈希、主要修改内容）；该文件自身的维护提交不记录
3. **发布伴随进度更新**：构建+部署（发布）后同步更新 `docs/PROGRESS.md`（已完成模块、改造记录、待办、最后更新日期）
4. 构建仅在用户明确要求时执行（见下）
...
**接口查询规范（强制）**：目标接口在 `cheng-swagger.json` 中查不到时，**先执行 `pm2 reload api-sync`**（重新同步后端 API 文档到本仓库），然后再查看文档——后端接口已更新但文档未同步属常见情况，禁止在未同步文档的情况下凭猜测实现接口。
